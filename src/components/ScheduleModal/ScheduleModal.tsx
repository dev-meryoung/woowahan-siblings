...
const ScheduleModal: React.FC<{
	...
	}, [isOpen, content, selectedSchedule, schedules]);

	const updateFormData = (field: string, value: string) => {
		setFormData((prevData) => ({ ...prevData, [field]: value }));
	};

	function resetForm(schedule: ISchedule | null = null) {
		if (schedule) {
	...
	function setFormValues(schedule: ISchedule) {
		const formattedDate = new Date(schedule.workDate).toISOString().split('T')[0];
		setFormData({
			workDate: formattedDate,
			wage: DEFAULT_DATA.wage,
	...
	function isFieldDisabled(): boolean {
		return content === 'view';
	}
	...
	async function handleAction(actionType: 'save' | 'update' | 'delete') {
		const fields = {
